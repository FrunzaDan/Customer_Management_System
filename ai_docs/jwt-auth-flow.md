...
- `API/.../CustomerManagementSystem.BusinessLogic/AuthFunctions/JwtSigningKey.cs` — the one shared place that turns `Auth:SecureJWTKey` into a `SymmetricSecurityKey`, used by both issuing and validating.
...
3. On success, a JWT is minted: symmetric **HMAC-SHA256** signing (`SymmetricSecurityKey` + `HmacSha256Signature`), key from `Auth:SecureJWTKey` in `appsettings.json` via `JwtSigningKey.Create` (committed value is a demo placeholder — see [[known-gaps]]).
...
- `JwtCreation.cs` and `Program.cs` used to each independently call `Encoding.ASCII.GetBytes(key)` to derive the signing key — two unsynchronized copies of the same logic, where "fixing" one (e.g. to properly base64-decode the key) without the other would have silently broken all authentication. Both now go through `JwtSigningKey.Create`, so there's structurally one implementation. The key is still raw ASCII bytes of the configured string, not base64-decoded, despite `SecureJwtKey` looking like base64 — that behavior itself wasn't changed, only the duplication.
